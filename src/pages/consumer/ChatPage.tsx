...
const ChatPage = () => {
  ...
  useEffect(() => {
    const initChat = async () => {
      ...
      // Inscrição Realtime robusta (Escuta TUDO da tabela e filtra no JS)
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            // Verifica se a mensagem pertence a esta conversa específica
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Não adicionamos manualmente aqui, pois o Realtime já fará isso para nós!
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-white z-[100] flex flex-col max-w-2xl mx-auto shadow-2xl">
      <header className="bg-white border-b border-gray-100 p-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <Button variant="ghost" size="icon" className="rounded-full text-indigo-600 bg-indigo-50">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-indigo-50/20">
        {messages.map((msg) => (
          <div 
            key={msg.id} 
            className={cn(
                "flex w-full flex-col animate-in fade-in slide-in-from-bottom-1", 
      ...
      <div className="p-4 bg-white border-t border-gray-100 safe-area-bottom">
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          <Input
            ...
            className="flex-1 rounded-2xl border-gray-100 bg-gray-50 h-12 focus:bg-white focus:ring-indigo-400"
            value={newMessage}
          ...
          <Button 
            type="submit" 
            size="icon" 
            className="h-12 w-12 rounded-2xl bg-brand-accent hover:bg-brand-accent/90 shadow-lg shadow-brand-accent/20" 
