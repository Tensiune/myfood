...
serve(async (req) => {
  ...
  try {
    ...
    if (order.driver_id || order.status === 'CANCELLED') {
      ...
      return new Response(JSON.stringify({ status: 'already_handled' }), {
        headers: corsHeaders
      })
    }
    ...
    if (isNaN(storeLat) || isNaN(storeLng)) {
      ...
      return new Response(JSON.stringify({ error: 'merchant_no_location' }), {
        status: 200,
        headers: corsHeaders
      })
    }
